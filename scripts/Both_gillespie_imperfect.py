...
#         print(len(t_steps))
        ...
        if max_infecteds > 100:
            num_sims += 1
            for ind, val in enumerate(t_steps):
                try:
                    average_rhos[sigma][val] += evolved_rhos[ind]
                except KeyError:
                    average_rhos[sigma][val] = evolved_rhos[ind]
                try:
                    num_infecteds_average[sigma][val] += infecteds[ind]
                except:
                    num_infecteds_average[sigma][val] = infecteds[ind]
...
#     flop
    ax[0][0].plot(t_steps, rho_values, label = fr"$\sigma$ = {sigma}", c = colours[i])
    ax[0][1].plot(t_steps, num_infected_values, label = fr"$\sigma$ = {sigma}", c = colours[i])
...
for i, zeta in enumerate(zetas):
    ...
    for j in range(repeats):
        ...
        if max_infecteds > 100:
            ...
            for ind, val in enumerate(t_steps):
                try:
                    average_rhos[zeta][val] += evolved_rhos[ind]
                except KeyError:
                    average_rhos[zeta][val] = evolved_rhos[ind]
                try:
                    num_infecteds_average[zeta][val] += infecteds[ind]
                except:
                    num_infecteds_average[zeta][val] = infecteds[ind]
...
#             print(t_steps[k], rho_values[k], t_step_vals[k])
    ...
    ax[1][0].plot(t_steps, rho_values, label = fr"$\zeta$ = {zeta}", c = colours[i])
    ax[1][1].plot(t_steps, num_infected_values, label = fr"$\zeta$ = {zeta}", c = colours[i])
...
#     print(num_infecteds_average[sigma])
    ...
    num_infected_values = [val/num_sims for val in list(num_infecteds_average[zeta].values())]
#     print(colours[i])
    ax[2][0].plot(t_steps, rho_values, label = fr"$\zeta$ = {zeta}", c = colours[i])
    ax[2][1].plot(t_steps, num_infected_values, label = fr"$\zeta$ = {zeta}", c = colours[i])
...
ax[0][0].set_xlim([0,t_max])
ax[1][0].set_xlim([0,t_max])
ax[2][0].set_xlim([0,t_max])
...
plt.savefig("../outputs/Gillespie_Models_imperfect.png")#, bbox_inches = "tight")
plt.savefig("../outputs/Gillespie_Models_imperfect.pdf")#, bbox_inches = "tight")
plt.close()