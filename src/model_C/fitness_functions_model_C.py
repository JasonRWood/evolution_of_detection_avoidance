...
def find_sing_strats(
    ...
    for i in range(resolution_inner + 1):
        ...
        if i != 0:
            ...
            if fit_grad_vec[-2] == 0 and fit_grad_vec[-1] == -1:
                poss_attractors.append([rhos[-2], rhos[-1]])
            if fit_grad_vec[-2] == 0 and fit_grad_vec == 1:
                poss_repellers.append([rhos[-2], rhos[-1]])

    # print(depth, depth_max, fit_grad_vec)
...
def fitness_gradient_rho(
    ...
    fit_grad_rho = (-S * eta * (delta - 1) * zeta * beta - S * ((-1 + (rho - 1) * (delta - 1) * eta) * zeta - (d + alpha + gamma)) * dbetadrho) / ((d + alpha + gamma) + zeta) / (d + alpha + gamma)
...
def R0_model_C(
    ...
    for i, rho in enumerate(evolved_rhos):
        ...
        R0 = ((1 - (delta - 1) * (-1 + rho) * eta) * zeta + d + gamma + alpha) * (b - d) / b / q * beta / (d + alpha + gamma) / (alpha + d + gamma + zeta)
        R0_no_evo = ((1 - (delta - 1) * (-1 + 0) * eta) * zeta + d + gamma + alpha) * (b - d) / b / q * beta_func(beta_max, 0, c1, c2) / (d + alpha + gamma) / (alpha + d + gamma + zeta)
