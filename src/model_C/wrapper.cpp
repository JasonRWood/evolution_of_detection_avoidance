...
namespace solvers{
    ...
    int Quick_solver::dynamics(float* dydt, float* y, int num_parasites, int* rho_inds,  float b, float q, float d, float *beta, float alpha, float gamma, float* rho, float delta_Q, float eta, float delta){
        ...
        float U[num_parasites], I[num_parasites];
        
        float Sdot, Rdot;
        float Q_dot[num_parasites], A_dot[num_parasites];
        float U_dot[num_parasites], I_dot[num_parasites];
        ...
        for(i=0;i<num_parasites;i++){
            ...
            I[i] = y[i + 1 + 3*num_parasites];
            
            Isum +=  Q[i] + A[i] + U[i] + I[i];
            
            infection_force_para[i] = beta[rho_inds[i]]*(A[i] + U[i] + I[i] + delta_Q*Q[i]);
        ...
        for(i = 0;i<num_parasites;i++){
            ...
            Q_dot[i] =  -(d + alpha + gamma)*Q[i] + delta*eta*I[i];
            A_dot[i] = -(d + alpha + gamma)*A[i] + delta*(1 - eta)*I[i];

            U_dot[i] = rho[rho_inds[i]]*infection_force_para[i]*S - (d + alpha + gamma)*U[i];

            I_dot[i] = (1 - rho[rho_inds[i]])*infection_force_para[i]*S - (d + alpha + gamma)*I[i] - delta*I[i];
        ...
        for(i = 0;i<num_parasites;i++){
            ...
            dydt[i + 1 + 3*num_parasites] = I_dot[i];
        }
        dydt[4*num_parasites + 1] = Rdot;
        return 0;
    }

    int Quick_solver::rkck(float *y, float *dydt, float *yout, float *yerr, float h, int num_parasites, int* rho_inds,  float b, float q, float d, float* beta, float alpha, float gamma, float* rho, float delta_Q, float eta, float delta)
    {
        int i;
        ...
        dynamics(y_1, y_temp, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);

        for(i=0;i<(2 + 4*num_parasites);i++){
            y_temp[i] = y[i]+h*(b31*dydt[i]+b32*y_1[i]);
        }
        
        dynamics(y_2, y_temp, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);
        
        for(i=0;i<(2 + 4*num_parasites);i++){
            y_temp[i]= y[i]+h*(b41*dydt[i]+b42*y_1[i]+b43*y_2[i]);
        }

        dynamics(y_3, y_temp, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);
        
        for(i=0;i<(2 + 4*num_parasites);i++){
            y_temp[i]= y[i]+h*(b51*dydt[i]+b52*y_2[i]+b53*y_2[i]+b54*y_3[i]);
        }

        dynamics(y_4, y_temp, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);
        
        for(i=0;i<(2 + 4*num_parasites);i++){
            y_temp[i]= y[i]+h*(b61*dydt[i]+b62*y_1[i]+b63*y_2[i]+b64*y_3[i]+b65*y_4[i]);
        }

        dynamics(y_5, y_temp, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);
    ...
    int Quick_solver::rkqs(float *y, float *dydt, float *h,float *hnext,float *yscale, int num_parasites, int* rho_inds,  float b, float q, float d, float* beta, float alpha, float gamma, float* rho, float delta_Q, float eta, float delta, float* t)
    {
        ...
        rkck(y, dydt, y_temp, yerr, *h, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);
        *hnext= *h;
    ...
    int Quick_solver::ad_dyn(float beta_max, float rho_max, float b, float q, float d, float alpha, float gamma, float c1, float c2, float delta_Q, float eta,  float delta, int seed, int rho_init, float S_density, float Q_density, float A_density, float U_density, float I_density, int evo_steps, int trait_space_length){
        ...
        float A_temp[trait_space_length], I_temp[trait_space_length];
        ...
        float para_output_Q_densities[num_poss_outputs], para_output_U_densities[num_poss_outputs], para_output_rho_vals[num_poss_outputs];
        float para_output_A_densities[num_poss_outputs], para_output_I_densities[num_poss_outputs];
        ...
        for (int evo_counter = 0; evo_counter < evo_steps; evo_counter++){
            ...
            while (t[0] <= tmax){
                ...
                dynamics(dydt, y, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta);
                ...
                rkqs(y, dydt, h,hnext,yScale, num_parasites, rho_inds, b, q, d, beta, alpha, gamma, rho, delta_Q, eta, delta, t);
            ...
            for(i=0;i<num_parasites;i++){
                ...
                if (temp_sum > TINY){
                    ...
                    I_temp[para_counter] = y[i + 1 + 3*num_parasites];
            ...
            for (i = 0; i<num_parasites_cleaned; i++){
                ...
                para_density[i] += A_temp[i] + I_temp[i];
                if (i == 0){
            ...
            for (i=0; i<num_parasites_cleaned; i++){
                ...
                y_temp[i + 3*num_parasites_2 + 1] = I_temp[i];
            ...
            for(i=0;i<num_parasites;i++){
                ...
                para_output_I_densities[para_output_counter] = y[i + 1 + 3*num_parasites];
        ...
        para_outputs << "Trait_index,host_density,density_of_Q,density_of_A,density_of_U,density_of_I,density_of_recovered_hosts,rho_value,beta_value,Evolutionary_step\n";
        for(i=0;i<para_output_counter;i++){
            para_outputs << para_output_rho_inds[i] <<"," << para_output_host_density[i] << "," << para_output_Q_densities[i] << "," << para_output_A_densities[i] << "," << para_output_U_densities[i] << "," << para_output_I_densities[i] << "," << para_output_R_densities[i] << "," << para_output_rho_vals[i] << ',' << beta[para_output_rho_inds[i]] << "," << para_output_evosteps[i] << "\n";
        }
        para_outputs.close();
    ...
    void Quick_solver::eco_dynamics(float* S, float* Q, float* A, float* U, float* I, float*R, float beta_value, float rho_value, float b, float q, float d, float alpha, float gamma, float c1, float c2, float delta_Q, float eta,  float delta, int seed, float S_density, float Q_density, float A_density, float U_density, float I_density){
