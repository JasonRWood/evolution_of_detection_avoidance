...
namespace solvers{
    ...
    void Quick_solver::aware_rate_function(float* aware_rate_array, int* Us, float* rhos, float zeta, int resolution){

        for(int i = 0; i < resolution + 1; i++){
            
            aware_rate_array[i] = Us[i]*(1 - rhos[i])*zeta;
        }
    ...
    void Quick_solver::run_gillespie_simulation(int seed, int N, int starting_infecteds, float beta_max, float c1, float c2, float zeta, float eta, float delta, float alpha, float gamma, float mut_chance, float t_max, int resolution, int S_increment){
        ...
        output_file.open("../data/gillespie_simulations_B/data_set" + seed_str +  ".csv");
        ...
        while(t_curr < t_max){
            ...
            aware_rate_function(aware_rate_array, Us, rhos, zeta, resolution);
            ...
            if (t_curr + t_inc > t_check){
                ...
                average_rho[0] = 0;
                for (i = 0; i < resolution + 1; i ++){
                    average_rho[0] += rhos[i]*float(Qs[i] + As[i] + Us[i])/total_pop;
                }
