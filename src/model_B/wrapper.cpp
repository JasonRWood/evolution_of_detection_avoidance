...
namespace solvers{
    ...
    int Quick_solver::dynamics(float* dydt, float* y, int num_parasites, int* rho_inds,  float b, float q, float d, float *beta, float alpha, float gamma, float* rho, float delta_Q, float eta, float delta){
        ...
        for(i=0;i<num_parasites;i++){
            ...
            infection_force_para[i] = beta[rho_inds[i]]*(A[i] + U[i] + delta_Q*Q[i]);
        ...
        for(i = 0;i<num_parasites;i++){
            ...
            Q_dot[i] =  -(d + alpha + gamma)*Q[i] + delta*(1-rho[rho_inds[i]])*eta*U[i];
            A_dot[i] = -(d + alpha + gamma)*A[i] + delta*(1-rho[rho_inds[i]])*(1 - eta)*U[i];

            U_dot[i] = infection_force_para[i]*S - (d + alpha + gamma)*U[i] - delta*(1 - rho[rho_inds[i]])*U[i];
    ...
    int Quick_solver::ad_dyn(float beta_max, float rho_max, float b, float q, float d, float alpha, float gamma, float c1, float c2, float delta_Q, float eta,  float delta, int seed, int rho_init, float S_density, float Q_density, float A_density, float U_density, int evo_steps, int trait_space_length){
        ...
        float para_output_A_densities[num_poss_outputs];
    ...
    void Quick_solver::eco_dynamics(float* S, float* Q, float* A, float* U, float*R, float beta_value, float rho_value, float b, float q, float d, float alpha, float gamma, float c1, float c2, float delta_Q, float eta,  float delta, int seed, float S_density, float Q_density, float A_density, float U_density){
