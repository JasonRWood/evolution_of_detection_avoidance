...
namespace solvers{
    ...
    void Quick_solver::infection_rate_function(float* infection_rate_array, int S, int* Qs, int* As, int* Us, int* Is, float* betas, float delta, int resolution){

        for (int i = 0; i< resolution + 1; i++){
            float Infection_rate;
            Infection_rate = betas[i]*(delta*Qs[i] + As[i] + Us[i] + Is[i])*S;
    ...
    void Quick_solver::recovery_rate_function(float* recovery_rate_array, int* Qs, int* As, int* Us, int* Is, float gamma, int resolution){

        for (int i = 0; i< resolution + 1; i++){
            float recovery_rate;
            recovery_rate = gamma*(Qs[i] + As[i] + Us[i] + Is[i]);
    ...
    void Quick_solver::mortality_rate_function(float* mortality_rate_array, int* Qs, int* As, int* Us, int* Is, float alpha, int resolution){

        for (int i = 0; i< resolution + 1; i++){
            float mortality_rate;
            mortality_rate = alpha*(Qs[i] + As[i] + Us[i] + Is[i]);
    ...
    void Quick_solver::aware_rate_function(float* aware_rate_array, int* Is, float* rhos, float zeta, int resolution){

        for(int i = 0; i < resolution + 1; i++){
            
            aware_rate_array[i] = Is[i]*zeta;
        }

        return;
    }

    void Quick_solver::run_gillespie_simulation(int seed, int N, int starting_infecteds, float beta_max, float c1, float c2, float zeta, float eta, float delta, float alpha, float gamma, float mut_chance, float t_max, int resolution, int S_increment){
        ...
        output_file.open("../data/gillespie_simulations_C/data_set" + seed_str +  ".csv");
        ...
        int* Is = new int[resolution + 1];

        int* proc_ind = new int[1];
        float* average_rho = new float[1];

        float* pops = new float[4];
        float* props = new float[4];
        ...
        // std::cout << t_curr << endl;
        // std::cout << "Max time" << endl;
        // std::cout << t_max << endl;
        ...
        float* aware_rate_array = new float[resolution + 1];
        
        float* total_prop_array = new float[4];
        float* total_rate_array = new float[4];
        while(t_curr < t_max){
            
            // std::cout << "Entered while loop" << endl;
            // float* infection_rate_array = new float[resolution + 1];
            // float* mortality_rate_array = new float[resolution + 1];
            // float* recovery_rate_array = new float[resolution + 1];
            // float* aware_rate_array = new float[resolution + 1];
            ...
            // output_file << k << ",";
            infection_rate_function(infection_rate_array, S, Qs, As, Us, Is, betas, delta, resolution);
            recovery_rate_function(recovery_rate_array, Qs, As, Us, Is, gamma, resolution);
            mortality_rate_function(mortality_rate_array, Qs, As, Us, Is, alpha, resolution);
            aware_rate_function(aware_rate_array, Is, rhos, zeta, resolution);

            // std::cout << "Made rates" << endl;
            float inf_sum = 0, rec_sum = 0, mort_sum = 0, aware_sum = 0;

            for (i = 0; i < resolution + 1; i++){
                inf_sum += infection_rate_array[i];
                rec_sum += recovery_rate_array[i];
                mort_sum += mortality_rate_array[i];
                aware_sum += aware_rate_array[i];
            }
            ...
            total_prop_array[0] = inf_sum/(inf_sum + rec_sum + mort_sum + aware_sum);
            total_prop_array[1] = (inf_sum + rec_sum)/(inf_sum + rec_sum + mort_sum + aware_sum);
            total_prop_array[2] = (inf_sum + rec_sum + mort_sum)/(inf_sum + rec_sum + mort_sum + aware_sum);
            total_prop_array[3] = 1.0;
            
            // std::cout << "Made prop arrays" << endl;
            ...
            if (t_curr + t_inc > t_check){
                ...
                if(proc_ind[0] == 0){
                        ...
                        float detect_rand = (float(rand())/float((RAND_MAX)));
                        ...
                        if (float(rand())/float((RAND_MAX)) < mut_chance){
                            if (infector_ind != 0 && infector_ind != resolution){
                                ...
                                if (mut_rand > 0.5){
                                    if(detect_rand < rhos[infector_ind]){                                        
                                        Us[infector_ind - 1] += 1;
                                    }
                                    else{
                                        Is[infector_ind - 1] += 1;
                    ...
                    if (proc_ind[0] == 1){
                        ...
                        int Q = Qs[rec_ind], A = As[rec_ind], U = Us[rec_ind], I = Is[rec_ind];
                        ...
                        props[0] = float(Q)/(Q + A + U + I);
                        props[1] = float(Q + A)/(Q + A + U + I);
                        props[1] = float(Q + A + U)/(Q + A + U + I);
                        props[3] = 1.0;
                    ...
                    if (proc_ind[0] == 2){
                        ...
                        int Q = Qs[die_ind], A = As[die_ind], U = Us[die_ind], I = Is[die_ind];
                    ...
                    if (proc_ind[0] == 3){
                        ...
                        float* aware_proportion_array = new float[resolution + 1];
                        create_proportions(aware_proportion_array, aware_rate_array, resolution);

                        int aware_ind = 0;
                        while(aware_proportion_array[aware_ind] < aware_rand){
                            aware_ind += 1;
                        }

                        // output_file << aware_rate_array[aware_ind] << ",";   
                        Is[aware_ind] += -1;

                        if (float(rand())/float((RAND_MAX)) > eta){
                            As[aware_ind] += 1;
                        }
                        else{
                            Qs[aware_ind] += 1;
                        }                
                        delete aware_proportion_array;
                    }
                // delete infection_rate_array;
                // delete aware_rate_array;
                // delete mortality_rate_array;
                // delete recovery_rate_array;

                float total_pop = 0;
                for (i = 0; i < resolution + 1; i ++){
                    total_pop += Qs[i] + As[i] + Us[i] + Is[i];
                }
                if (total_pop == 0){
                ...
                average_rho[0] = 0;
                for (i = 0; i < resolution + 1; i ++){
                    average_rho[0] += rhos[i]*float(Qs[i] + As[i] + Us[i] + Is[i])/total_pop;
                }
                // evolved_rhos[k] = average_rho;
